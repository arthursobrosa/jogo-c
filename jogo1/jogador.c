...
/*void UsarItem(Jogador *jogador, EstadoJogo *estado)
{
    ...
    {
        case ITEM_ARMA:
            // Lógica: Criar um novo projétil na direção do angulo do jogador.
            // CriarProjetil(jogador->celula.retangulo, jogador->celula.angulo);
            break;

        case ITEM_ESCUDO:
            // Lógica: Ativar o flag de escudo e o temporizador.
            jogador->escudoAtivo = true;
            jogador->tempoEscudo = CONST_DURACAO_ESCUDO; // CONST_DURACAO_ESCUDO precisa ser definida
            break;

        case ITEM_BOMBA:
            // Lógica: Criar um novo objeto Bomba na posição de retaguarda.
            // CriarBomba(CalcularPosicaoRetaguarda(jogador->celula.retangulo, jogador->celula.angulo));
            break;

        case ITEM_BONUS_VELOCIDADE:
             // Lógica: Aumentar temporariamente o tempoVelocidade
            jogador->tempoVelocidade = CONST_DURACAO_BONUS;
            // NOTE: A velocidade real é aplicada na função de física
            break;
    }
